...
BOOL CMarkupMSXML::s_InsertSubDoc( LPCTSTR szSubDoc, MSXMLNS::IXMLDOMNodePtr pParent, MSXMLNS::IXMLDOMNodePtr pInsertBefore )
{
#if defined(MARKUP_MSXML4)
	...
	x_Insert( pParent, pInsertBefore, pNew );
...
///////////////////////////////////////////////////////////////////////////////////////////
// class CTWenXMLSingle
///////////////////////////////////////////////////////////////////////////////////////////

void CTWenXMLSingle::Reset()
{
	for(int i = Children.GetCount()-1; i>-1; i--) delete Children.GetAt(i);
	Children.RemoveAll();
}
//
CString CTWenXMLSingle::GetNodeName() const
{
	return m_pRoot->xml.x_GetTagName(m_pNode);
}
CString CTWenXMLSingle::GetProperty(LPCTSTR propName) const
{
	return m_pRoot->xml.x_GetAttrib(m_pNode, propName);
}
void CTWenXMLSingle::SetProperty(LPCTSTR propName, LPCTSTR propVal)
{
	m_pRoot->xml.x_SetAttrib(m_pNode, propName, propVal);
}
//
CString CTWenXMLSingle::GetInnerXML() const
{
	CString ret;
	for(int i=0; i<Children.GetCount(); i++)
	{
		ret = ret + Children.GetAt(i)->GetOuterXML() + _T("\r\n");
	}
	return ret;
}
CString CTWenXMLSingle::GetOuterXML() const
{
	return CMarkupMSXML::FromBSTR( m_pNode->xml );
}
CString CTWenXMLSingle::GetInnerText() const
{
	return m_pRoot->xml.x_GetData(m_pNode);
}

void CTWenXMLSingle::SetInnerXML(LPCTSTR xml)
{
	DeleteChildren();
	m_pRoot->xml.s_InsertSubDoc(xml, m_pNode, NULL);
	s_SetupXML(this, m_pNode, false);
}
void CTWenXMLSingle::SetInnerText(LPCTSTR text)
{
	m_pRoot->xml.x_SetData(m_pNode, text, NULL);
}
//
void CTWenXMLSingle::AppendChild(LPCTSTR xml)
{
	m_pRoot->xml.s_InsertSubDoc(xml, m_pNode, NULL);
	//递归xml,建立子元素
	s_SetupXML(this, m_pNode, true);
}
void CTWenXMLSingle::s_SetupXML(CTWenXMLSingle* pSingle, MSXMLNS::IXMLDOMNodePtr pNode, bool bCheckChildren)
{
	MSXMLNS::IXMLDOMNodePtr pChildNode = pNode->GetfirstChild();
	while(pChildNode)
	{
		if ( pChildNode->nodeType == MSXMLNS::NODE_ELEMENT )
		{
			if(bCheckChildren)
			{
				bool bFound = false;
				...
				{
					if(pSingle->Children.GetAt(i)->m_pNode == pChildNode)
					{
						bFound = true;
						break;
					}
			...
			CTWenXMLSingle* pChildSingle = new CTWenXMLSingle(pSingle->m_pRoot, pSingle, pChildNode);
			pSingle->Children.Add(pChildSingle);
			s_SetupXML(pChildSingle, pChildNode, false);
		}
		pChildNode = pChildNode->GetnextSibling();
	}
}
void CTWenXMLSingle::Delete()
{
	if(m_pParent) m_pParent->DeleteChild(this);
	else if(m_pRoot) m_pRoot->DeleteChild(this);
}
void CTWenXMLSingle::DeleteChild(CTWenXMLSingle* pChild)
{
	for(int i=0; i<Children.GetCount(); i++)
	{
		if(Children.GetAt(i) == pChild)
		{
			...
			if ( pChild->m_pNode )
			{
				MSXMLNS::IXMLDOMNodePtr pParent = pChild->m_pNode->GetparentNode();
				if ( pParent )
				{
					pParent->removeChild( pChild->m_pNode );
					pChild->m_pNode = NULL;
				}
			}
			delete pChild;
...
void CTWenXMLSingle::DeleteChildren()
{
	for(int i=0; i<Children.GetCount(); i++)
	{
		CTWenXMLSingle* pChild = Children.GetAt(i);
		if ( pChild->m_pNode )
		{
			MSXMLNS::IXMLDOMNodePtr pParent = pChild->m_pNode->GetparentNode();
			if ( pParent )
...
bool CTWenXMLBase::LoadFromFile(LPCTSTR xmlFile)
{
	Reset();
	if(!xml.Load(xmlFile)) return false;
	while(xml.FindElem())
	{
		CTWenXMLSingle* pSingle = new CTWenXMLSingle(this, NULL, xml.m_pMain);
		Siblings.Add(pSingle);
		CTWenXMLSingle::s_SetupXML(pSingle, xml.m_pMain, false);
	}
	return true;
}
bool CTWenXMLBase::LoadFromString(LPCTSTR xmlString)
{
	Reset();
	if(!xml.SetDoc(xmlString)) return false;
	while(xml.FindElem())
	{
		CTWenXMLSingle* pSingle = new CTWenXMLSingle(this, NULL, xml.m_pMain);
		Siblings.Add(pSingle);
...
void CTWenXMLBase::CopyFrom(CTWenXMLSingle* pSingle)
{
	LoadFromString(pSingle->GetOuterXML());
}
bool CTWenXMLBase::CopyTo(LPCTSTR path, CTWenXMLBase& base)
{
	CTWenXMLSingle* pSingle = Path(path);
	if(!pSingle) return false;
	base.CopyFrom(pSingle);
	return true;
}

CString CTWenXMLBase::GetDoc() const
{
	return xml.GetDoc();
}

CTWenXMLSingle* CTWenXMLBase::Path(LPCTSTR path) const
{
	CString p(path);
	p.Replace(_T('/'), _T('\\'));
	...
	ext.BeginEnum();
	CString cur;
	CTWenXMLSingle* pSingle = NULL;
	const CArray<CTWenXMLSingle*, CTWenXMLSingle*>* pEnum = &Siblings;
	while(ext.GetNext(cur))
	{
		CTWenXMLSingle* pEnumSingle = NULL;
		for (int i=0; i<pEnum->GetCount(); i++)
		{
			pEnumSingle = pEnum->GetAt(i);
			if(pEnumSingle->GetNodeName() == cur)
			{
				pSingle = pEnumSingle;
				break;
			}
		}
		if(pEnumSingle == NULL) return NULL;
		if(pSingle != pEnumSingle) return NULL;
		pEnum = &(pSingle->Children);
	}
	return pSingle;
}
void CTWenXMLBase::Reset()
{
	for(int i = Siblings.GetCount()-1; i>-1; i--) delete Siblings.GetAt(i);
	Siblings.RemoveAll();
}

void CTWenXMLBase::DeleteChild(CTWenXMLSingle* pChild)
{
	for(int i=0; i<Siblings.GetCount(); i++)
	{
		if(Siblings.GetAt(i) == pChild)
		{
			Siblings.RemoveAt(i);
			if ( pChild->m_pNode )
			{
