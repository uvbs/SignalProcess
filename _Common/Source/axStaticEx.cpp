...
//        m_hCurCursor=AfxGetApp()->LoadStandardCursor(IDC_HAND);	//PHH加入
        m_hCurCursor=AfxGetApp()->LoadStandardCursor(IDC_CROSS);	//PHH加入
...
*/
	...
	//将漂白的面积扩大一些,以免在压缩时边界产生杂色
	m_rectTracker.left -= 10/dblZoomX; 
	if(m_rectTracker.left<0) m_rectTracker.left=0;
	m_rectTracker.right += 10/dblZoomX; 
	if(m_rectTracker.right>m_pxImgInner->GetWidth()) m_rectTracker.right=m_pxImgInner->GetWidth();
	m_rectTracker.bottom -= 10/dblZoomY; 
	if(m_rectTracker.bottom<0) m_rectTracker.bottom=0;
	m_rectTracker.top += 10/dblZoomY; 
	if(m_rectTracker.top>m_pxImgInner->GetHeight()) m_rectTracker.top=m_pxImgInner->GetHeight();
...
//	AfxMessageBox(cStrTemp);
	...
	if(m_rectTracker.Width()>=PHOTO_ORI_POINTS_X||m_rectTracker.Height()>=PHOTO_ORI_POINTS_Y) {
		AfxMessageBox(_T("错误号013：照片像素过多!!请在相机设置中增大“照片像素调整系数”，或者在拍摄前缩小取景框（拍摄区内）"));
		return;
	}
	...
	for(;;) {
		...
		for(int i=m_rectTracker.right-1;i>=m_rectTracker.left+1; --i) {
			for(int j=m_rectTracker.top-1;j>=m_rectTracker.bottom+1;--j) {
				//这一点符合要求
				if(m_ArrayPoint[i-m_rectTracker.left][j-m_rectTracker.bottom] == 1) {
					//以这一点的颜色作为标准能够对付渐变的背景
					m_rgbOld= m_pxImgInner->GetPixelColor(i, j);//获得颜色
