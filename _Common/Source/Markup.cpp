...
CString CMarkup::x_GetData( int iPos ) const
{
	...
	//if ( ! m_aPos[iPos].iElemChild && ! m_aPos[iPos].IsEmptyElement() )
	{
				...
				&& _tcsncmp( &((LPCTSTR)m_csDoc)[token.nL+1], _T("![CDATA["), 8 ) == 0 )
		{
			...
			{
				CString strtmp = m_csDoc.Mid( token.nL+9, nEndCDATA-token.nL-9 );
				return strtmp;
			}
		}
		CString strText = x_TextFromDoc( m_aPos[iPos].nStartR+1, m_aPos[iPos].nEndL-1 );
		return strText;
	}
	return _T("");
}
...
CString CMarkup::x_TextToDoc( LPCTSTR szText, bool bAttrib ) const
{
	...
	while ( cSource )
	{
		...
		if ( (pFound=(LPTSTR)_tcschr(pFind,cSource)) != NULL )
		{
