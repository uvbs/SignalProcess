...
///////////////////////////////////////////////////////////////////////////
// CAxRectTracker global state
// various GDI objects we need to draw
static HCURSOR s_afxCursors[10] = { 0, };
static HBRUSH  s_afxHatchBrush = 0;
static HPEN    s_afxBlackDottedPen = 0;
static int     s_afxHandleSize = 0;

// the struct below is used to determine the qualities of a particular handle
struct AX_HANDLEINFO
{
...
static const AX_HANDLEINFO s_afxHandleInfo[] =
{
...
static const AX_RECTINFO s_afxRectInfo[] =
{
...
///////////////////////////////////////////////////////////////////////////
// CAxRectTracker intitialization
CAxRectTracker::CAxRectTracker()
{
    Construct();    
}

CAxRectTracker::CAxRectTracker(const RECT* pSrcRect, UINT nStyle)
{
	ASSERT(AfxIsValidAddress(pSrcRect, sizeof(RECT), FALSE));
	Construct();
	m_rect.CopyRect(pSrcRect);
	m_nStyle = nStyle;
}
...
BOOL CAxRectTracker::SetCursor(CWnd* pWnd, UINT nHitTest) const
{
	...
	ASSERT(nHandle < _countof(s_afxCursors));
	::SetCursor(s_afxCursors[nHandle]);
	return TRUE;
}
...
int CAxRectTracker::NormalizeHit(int nHandle) const
{
	ASSERT(nHandle <= 8 && nHandle >= -1);
	if (nHandle == hitMiddle || nHandle == hitNothing)
		return nHandle;
	const AX_HANDLEINFO* pHandleInfo = &s_afxHandleInfo[nHandle];
	if (m_rect.Width() < 0)
	{
		nHandle = (TrackerHit)pHandleInfo->nInvertX;
		pHandleInfo = &s_afxHandleInfo[nHandle];
	}
	if (m_rect.Height() < 0)
...
BOOL CAxRectTracker::TrackRubberBand(CWnd* pWnd, CPoint point, BOOL bAllowInvert)
{
...
void CAxRectTracker::DrawTrackerRect(LPCRECT lpRect, CWnd* pWndClipTo, CDC* pDC, CWnd* pWnd)
{
...
void CAxRectTracker::AdjustRect(int nHandle, LPRECT)
{
	...
	if (px != NULL && nAbsWidth < m_sizeMin.cx)
	{
		...
		ASSERT((int*)px - (int*)&m_rect < _countof(s_afxRectInfo));
		const AX_RECTINFO* pRectInfo = &s_afxRectInfo[(int*)px - (int*)&m_rect];
	...
	if (py != NULL && nAbsHeight < m_sizeMin.cy)
	{
		...
		ASSERT((int*)py - (int*)&m_rect < _countof(s_afxRectInfo));
		const AX_RECTINFO* pRectInfo = &s_afxRectInfo[(int*)py - (int*)&m_rect];
		*py = *(int*)((BYTE*)&m_rect + pRectInfo->nOffsetAcross) +
			nNewHeight * m_sizeMin.cy * -pRectInfo->nSignAcross;
	}
}
void CAxRectTracker::GetTrueRect(LPRECT lpTrueRect) const
{
...
void CAxRectTracker::OnChangedRect(const CRect& /*rectOld*/)
{
	// no default implementation, useful for derived classes
}
///////////////////////////////////////////////////////////////////////////
// CAxRectTracker implementation helpers
void CAxRectTracker::GetHandleRect(int nHandle, CRect* pHandleRect) const
{
	ASSERT(nHandle < 8);
...
int CAxRectTracker::GetHandleSize(LPCRECT lpRect) const
{
	if (lpRect == NULL)
		lpRect = &m_rect;
...
int CAxRectTracker::HitTestHandles(CPoint point) const
{
	CRect rect;
	UINT mask = GetHandleMask();
...
BOOL CAxRectTracker::TrackHandle(int nHandle, CWnd* pWnd, CPoint point,
	CWnd* pWndClipTo)
{
	ASSERT(nHandle >= 0);
...
void CAxRectTracker::GetModifyPointers(
...
UINT CAxRectTracker::GetHandleMask() const
{
...
/////////////////////////////////////////////////////////////////////////////
// CAxRectTracker intitialization
//CAxRectTracker::CAxRectTracker(LPCRECT lpSrcRect, UINT nStyle)
//{
...
//	return nHandle;
//}
//BOOL CAxRectTracker::Track(CWnd* pWnd, CPoint point, BOOL bAllowInvert,
//	CWnd* pWndClipTo)
//{
//	// perform hit testing on the handles
//	int nHandle = HitTestHandles(point);
...
//	return TrackHandle(hitBottomRight, pWnd, point, NULL);
//}
//void CAxRectTracker::DrawTrackerRect(
