...
void COutlook2Ctrl::SetupColors()
{
	...
	m_crBackCaption = GetSysColor(COLOR_BTNFACE);
	//m_crTextCaption = GetSysColor(COLOR_WINDOW);
	m_crTextCaption = RGB(0,0,0);
...
{
#ifdef __BCGCB_H
	if (OLCTRL2_BASE_CLASS::Create(pParent,WS_VISIBLE|WS_CHILD|CBRS_LEFT|WS_HSCROLL,iId))
#else
	if (CControlBar::Create(NULL, _T(""), WS_VISIBLE|WS_CHILD|WS_HSCROLL, CRect(0,0,0,0), pParent, iId))
#endif
	{
		SetOwner(pParent);
		return true;
...
int COutlook2Ctrl::AddFolder(LPCTSTR m_strName, HICON hIcon, DWORD lParam)
{
	if (m_csCaption == _T("")) m_csCaption = m_strName;
...
int COutlook2Ctrl::AddFolderItem(LPCTSTR m_strItemName, DWORD dwStyle, int iFolder)
{
...
void COutlook2Ctrl::ChangeFolderItemName(CString cStrFolderName, CString cStrName2Change)
{
	for (int t = (int) m_Folders.GetSize() - 1; t >= 0; t--){
		COL2Folder * o = (COL2Folder *) m_Folders.GetAt(t);
		if(o->csName==cStrFolderName) {
			if(o->m_Items.GetSize()>0) {
				//仅改变第一个的名称
				COL2Item * i = (COL2Item *)(o->m_Items[0]);
				i->csName=cStrName2Change;
			}
		}
	}
}
...
int COutlook2Ctrl::AddSubItem(LPCTSTR m_strName, HICON hIcon, DWORD dwStyle, DWORD lParam, int iFolder, int iFolderItem)
{
	if (iFolder < 0) iFolder = (int) m_Folders.GetSize() - 1;
	COL2Folder * o = (COL2Folder *) m_Folders.GetAt(iFolder);
...
void COutlook2Ctrl::DrawButtons(CDC * pDC, CRect & rect)
{
	...
	for (int t = (int) m_Folders.GetSize() - 1; t >= 0; t--)
	{
		...
		pDC->FillSolidRect(o->rcItem.left,o->rcItem.top,o->rcItem.Width(), o->rcItem.Height() + 1, m_crBackCaption);
...
void COutlook2Ctrl::DrawButton(CDC * pDC, COL2Folder * o, bool bSel, bool bOver)
{
	...
	if (bSel) 
	{
		...
		COLORREF cr2 = GetSysColor(COLOR_ACTIVECAPTION);
	...
	{
#if(WINVER >= 0x0500)
		COLORREF cr1 = GetSysColor(COLOR_GRADIENTINACTIVECAPTION);
		COLORREF cr2 = GetSysColor(COLOR_INACTIVECAPTION);
		DrawGradientRect(pDC, rect, cr1, cr2);
	...
	if (o->hIcon && rect.left+20 < rect.right) DrawIconEx(pDC->GetSafeHdc(), rect.left+4, rect.top+1, o->hIcon,16,16,0,NULL, DI_NORMAL);
