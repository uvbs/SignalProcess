...
class CMarkupMSXML  
{
	BOOL s_InsertSubDoc( LPCTSTR szSubDoc, MSXMLNS::IXMLDOMNodePtr pParent, MSXMLNS::IXMLDOMNodePtr pInsertBefore );
public:
...
protected:
	...
	friend class CTWenXMLNode;
	friend class CTWenXMLBase;
	friend class CTWenXMLSingle;
};

class CTWenXMLSingle
{
protected:
	CTWenXMLSingle(CTWenXMLBase* pRoot,CTWenXMLSingle* pParent, MSXMLNS::IXMLDOMNodePtr pNode)
		:m_pRoot(pRoot), m_pParent(pParent), m_pNode(pNode){}
	friend class CTWenXMLBase;

public:
	~CTWenXMLSingle(void){Reset();}
...
public:
	...
	void SetInnerXML(LPCTSTR xml);
	void SetInnerText(LPCTSTR text);

	//	void BeginEnumProperty();
	//	bool EnumProperty();
	//	void GetCurProperty(CString* propName, CString* propVal)const;
	//	void SetCurProperty(CString* propVal);

	void AppendChild(LPCTSTR xml);
	void Delete();
	void DeleteChildren();

	CTWenXMLSingle* GetParent() const{return m_pParent;}
	CTWenXMLBase* GetRoot() const{return m_pRoot;}

public:
	CArray<CTWenXMLSingle*, CTWenXMLSingle*> Children;

protected:
	void Reset();
	void DeleteChild(CTWenXMLSingle* pChild);

protected:
	static void s_SetupXML(CTWenXMLSingle* pSingle, MSXMLNS::IXMLDOMNodePtr pNode, bool bCheckChildren);

protected:
	MSXMLNS::IXMLDOMNodePtr m_pNode;
	CTWenXMLSingle* m_pParent;
	CTWenXMLBase* m_pRoot;
};
...
public:
	CTWenXMLBase(void){}
	~CTWenXMLBase(void){Reset();}

public:
	bool LoadFromFile(LPCTSTR xmlFile);
	bool LoadFromString(LPCTSTR xmlString);
	void CopyFrom(CTWenXMLSingle* pSingle);
	bool CopyTo(LPCTSTR path, CTWenXMLBase& base);

	CString GetDoc() const;//是否可能包含xml文件头取决于加载值

	bool Save(LPCTSTR xmlFile){return xml.Save(xmlFile) == TRUE;}

	CTWenXMLSingle* Path(LPCTSTR path) const;

public:
	CArray<CTWenXMLSingle*, CTWenXMLSingle*> Siblings;
